...
import animation_rules
import shared
from animation_rules import PROJECT_DIRECTORY, TOKEN, kill_token_processes
from assets.animations.cat_animations import cat_animations_list
from assets.animations.info_animations import info_no_sound, info_sound
...
from shared import check_sound_and_player_status, show_help

proc_stop_event: Event | None = None
splash_animation_index = 0

options = ["cat", "waves", "cava", "info", "splash", "empty", "flat"]
options_with_values = ["cava", "flat", "empty"]
option_values = {
    "empty_values": {
...
def kill_cava(category, pid, proc_stop_event):
    while True:
        sound, player = check_sound_and_player_status()
        if (
            (category == "off" and player is True)
            or (category == "inactive" and (sound is True or player is False))
            or (category == "active" and (sound is False or player is False))
            or proc_stop_event.is_set()
        ):
            pid.kill()
            break
        sleep(1)
...
class Show(object):
    ...
    def show_cava(category):
        ...
        cache_files = str(
            subprocess.check_output([f"ls {cache_dir}| wc -l 2>/dev/null"], shell=True)
        )[2:-3]
        ...
        global proc_stop_event
        proc_stop_event = threading.Event()

        cava_position = option_values["cava_values"][f"{category}_cava_sections"]
        play_cava = PROJECT_DIRECTORY + "/scripts/play_cava.sh"
        ...
        try:
            ...
            thread2 = threading.Thread(
                target=kill_cava, args=(category, proc, proc_stop_event)
            )
...
if __name__ == "__main__":
    try:
        ...
        events_list = [
            proc_stop_event,
            animation_rules.animation_stop_event,
            animation_rules.full_animation_stop_event,
        ]
